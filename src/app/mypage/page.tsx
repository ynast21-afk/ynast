                        ...
                        <div className="w-full md:w-auto flex flex-col gap-3">
                            ...
                            <button
                                onClick={async () => {
                                    ...
                                    if (isCancelled) {
                                        // 복구 플로우
                                        if (confirm('자동 결제를 다시 활성화하시겠습니까?\n\n다음 결제일에 자동으로 결제가 진행됩니다.')) {
                            ...
                            >
                                {isCancelling ? '처리 중...' : user.subscriptionCancelled ? '🔄 자동 결제 다시 활성화' : '🔄 자동 결제 해지'}
                            </button>
                        </div>
                    )}
                            ...
                            downloads.map((video: any) => (
                                <div key={video.id} className="group bg-bg-secondary rounded-xl overflow-hidden border border-white/5 p-4 flex gap-4 items-center">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-text-tertiary text-xs">@{video.streamerName}{(() => { const s = streamers.find((st: any) => st.name === video.streamerName); return s?.koreanName ? ` (${s.koreanName})` : ''; })()} • {video.duration}</p>
