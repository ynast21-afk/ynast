...
export async function POST(request: NextRequest) {
    try {
        ...
        // Validate file size (max 5MB for preview images)
        ...
        const ext = fileType === 'image/gif' ? 'gif' : 'png'
        const fileName = `preview_gif/${videoId}.${ext}`
        ...
        // Update video record with previewGifUrl (non-blocking, best-effort)
        try {
            ...
            console.warn('Failed to save previewGifUrl to DB (non-critical):', dbErr)
        }
