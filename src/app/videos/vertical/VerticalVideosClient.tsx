...
interface VerticalVideosClientProps {
    ssrVideos: Video[]
    streamerNames: string[]
}

export default function VerticalVideosClient({ ssrVideos, streamerNames }: VerticalVideosClientProps) {
    ...
    const allVideos = !isLoading && contextVideos.length > 0 ? contextVideos : ssrVideos

    // Filter to vertical videos only
    const verticalVideos = allVideos.filter(v => v.orientation === 'vertical')

    // Apply filters
    const filteredVideos = verticalVideos.filter(video => {
    ...
    // Unique streamers from context (live data) for filter
    const liveStreamerNames = useMemo(() => {
        const names = Array.from(new Set(
            allVideos
                .filter(v => v.orientation === 'vertical')
                .map(v => v.streamerName)
                .filter(Boolean)
        ))
        return names.length > 0 ? names : streamerNames
    }, [allVideos, streamerNames])
    ...
    return (
        <>
            ...
            {/* Videos Grid - Portrait optimized layout */}
